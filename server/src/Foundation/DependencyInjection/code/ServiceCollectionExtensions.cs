...
namespace MockProject.Foundation.DependencyInjection
{
	public static class ServiceCollectionExtensions
	{
		...
		public static void AddControllersInCurrentAssembly(this IServiceCollection serviceCollection, params string[] classFilters)
		{
			AddControllers(serviceCollection, Assembly.GetCallingAssembly());
		}

		public static void AddControllers(this IServiceCollection serviceCollection, params string[] assemblyFilters)
		{
			serviceCollection.AddControllers(GetAssemblies(assemblyFilters));
		}

		public static void AddControllers(this IServiceCollection serviceCollection, params Assembly[] assemblies)
		{
			serviceCollection.AddControllers(assemblies, new[] { DefaultControllerFilter });
		}

		public static void AddControllers(this IServiceCollection serviceCollection, string[] assemblyFilters, params string[] classFilters)
		{
			serviceCollection.AddControllers(GetAssemblies(assemblyFilters), classFilters);
		}

		private static void AddControllers(this IServiceCollection serviceCollection, IEnumerable<Assembly> assemblies, string[] classFilters)
		{
			// register Mvc Controller
			var controllers = GetTypesImplementing(typeof(IController), assemblies, classFilters);
			var apiControllers = GetTypesImplementing(typeof(IHttpController), assemblies, classFilters);
		...
		private static IEnumerable<Type> GetExportedTypes(Assembly assembly)
		{
			...
			catch (Exception ex)
			{
				...
				var newExceptionMessage = string.Format(CultureInfo.InvariantCulture, "Unable to load types from assembly {0}. {1}", nameof(assembly), ex.Message);
				throw new InvalidOperationException(newExceptionMessage, ex);
			}
		}
